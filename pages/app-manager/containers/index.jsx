...
export default function Container() {
  ...
  useEffect(() => {
    ...
    pagination.perpage,
    search,
    state.sortItems["Container"],
  ]);

  //   useEffect(() => {
  //     const socket = getSocket();
  //     socket.on("Container:create", fetchTableData);
  //     socket.on("Container:update", fetchTableData);
  //     socket.on("Container:delete", fetchTableData);
  //   }, []);

  const fetchTableData = async () => {
    setTableData([]);
    dispatch({ type: "SET_STATE", payload: { loading: true } });
    const [response, err] = await http.get(`${domain}/containers`, {
      params: {
        ...pagination,
        search,
      },
  ...
  return (
    <div className="p-10 md:ml-20 mb-20 m-0">
      ...
      <div className="flex flex-wrap mb-5">
        ...
        <Button onClick={() => router.push("/app-manager/containers/new")}>
          Add Container
        </Button>
      </div>
      ...
      <Table
        ...
        colEvents={{
          name: {
            onClick: (e, item) => {
              router.push(`/app-manager/containers/${item._id}`);
            },
          },
        }}
      ></Table>
